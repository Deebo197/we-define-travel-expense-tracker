...
import {
  ...
  CreditCard, FileText, MapPin, Building2, X, LogOut, Inbox, HelpCircle } from
"lucide-react";
...
{ path: "/help", label: "Help & Guide", icon: HelpCircle, adminOnly: false }];
...
export default function Sidebar({ onClose }) {
  ...
  return (
    <div className="h-full flex flex-col theme-sidebar">
        ...
        {filteredNav.map((item) => {
          ...
          return (
            <Link
              ...
              onMouseEnter={(e) => {if (!isActive) {e.currentTarget.style.backgroundColor = "var(--bg-surface-2)";e.currentTarget.style.color = "var(--text-secondary)";}}}
              onMouseLeave={(e) => {if (!isActive) {e.currentTarget.style.backgroundColor = "transparent";e.currentTarget.style.color = "var(--text-tertiary)";}}}>
      ...
      {/* User info */}
      <div className="p-4 theme-border-soft" style={{ borderTop: "1px solid var(--border-soft)" }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium truncate theme-text-primary">{user?.full_name || "Loading..."}</div>
